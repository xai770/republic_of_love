...
class GemmaConciseExtractor:
    ...
    def __init__(self, model_name: str = "qwen3:latest"):
        ...
        if self.is_available:
            ...
            # Display the exact prompt template on first initialization
            print("\n" + "="*80)
            print("📋 V11.0 PROMPT TEMPLATE (2025-07-20 VALIDATED)")
            print("="*80)
            print(self.template_prompt)
            print("="*80)
            print("📝 [JOB DESCRIPTION WILL BE INSERTED HERE]")
            print("="*80 + "\n")
        else:
...
```"""
    ...
    def extract_concise_description(self, job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ V11.0 extraction failed: {str(e)}")
