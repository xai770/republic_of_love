...
class MinimalExtractor:
    ...
    def extract_job_data(self, job_description: str, job_title: str = "") -> Dict[str, Any]:
        ...
        if self.llm_available:
            try:
                ...
                # Try V11.0 qwen3-based extraction
    ...
    def _create_concise_description(self, description: str, job_title: str) -> str:
        ...
        if self.llm_available:
            try:
                logger.info(f"🤖 Using V11.0 LLM for concise description extraction: {job_title}")
                
                # Import the V11.0 Gemma extractor
                from gemma_concise_extractor_v11 import GemmaConciseExtractor
                
                # Use the V11.0 qwen3:latest process with 2025-07-20 template
                extractor = GemmaConciseExtractor(model_name="qwen3:latest")
                ...
                if extractor.is_ready():
                    ...
                    if result.get("status") == "success" and result.get("concise_description"):
                        ...
                        # V11.0: Return the structured "Your Tasks"/"Your Profile" format directly
                        if concise_desc and len(concise_desc) > 20:
                            return concise_desc  # No "Role Overview:" prefix - return structured format as-is
                    
                    # V11.0: Legacy warning removed
                else:
                ...
                if len(extracted) > 50:  # Meaningful content
                    ...
                    if len(cleaned) > 2000:
                        ...
                        if last_period > 1500:  # Ensure we have substantial content
                            ...
                            cleaned = truncated + "..."
                    return f"**Role Summary:** {cleaned}"  # V11.0: Changed from "Role Overview" for fallback
