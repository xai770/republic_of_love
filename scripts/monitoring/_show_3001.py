...
def get_posting_counts(conn):
    ...
    # Get basic counts from postings table
    cursor.execute("""
        SELECT 
            COUNT(*) as total,
            COUNT(*) FILTER (WHERE invalidated = false) as active,
            COUNT(extracted_summary) FILTER (WHERE invalidated = false) as has_summary,
            COUNT(ihl_score) FILTER (WHERE invalidated = false) as has_ihl
        FROM postings
    ...
    # Get skills count from posting_skills table (more accurate than skill_keywords column)
    cursor.execute("""
        SELECT COUNT(DISTINCT ps.posting_id)
        FROM posting_skills ps
        JOIN postings p ON p.posting_id = ps.posting_id
        WHERE p.invalidated = false AND p.source = 'deutsche_bank'
    """)
...
def print_step(conv_id, name, actor, actor_type, stats, phosphor, latencies, total_postings, max_phosphor, step_num):
    ...
    if is_conditional:
        ...
        pct = "cond "  # Mark as conditional (trailing space for alignment)
    else:
        pct_value = 100*completed/total_postings if total_postings > 0 else 0
        pct = f"{pct_value:3.0f}% "  # Trailing space for alignment
    ...
    if is_conditional:
        progress_str = f"{completed:4d}      {pct}"  # 4 + 6 + 4 = 14 + space = 15
    else:
        progress_str = f"{completed:4d}/{total_postings:<4d} {pct}"  # 4 + 1 + 4 + 1 + 4 = 14 + space = 15
    
    # phosphor_str contains ANSI codes, so can't use :Ns formatting. It's already fixed width visually (21 chars)
    print(f"{step_num:2d} {icon} {name:35s} | {actor:18s} | {time_str:>6s} | {eta_str:>6s} | {progress_str:15s} | {phosphor_str} | {status}")
...
def print_flowchart_dashboard():
    ...
    # Check if all key outputs are complete (summaries, skills, IHL all at 100%)
    all_complete = (
        posting_counts['has_summary'] >= posting_counts['active'] and
        posting_counts['has_skills'] >= posting_counts['active'] and
        posting_counts['has_ihl'] >= posting_counts['active']
    )
    
    if all_complete or total_eta_seconds <= 0:
        eta_str = "\033[1;32mnow ✅\033[0m"
    elif total_eta_seconds > 0:
    ...
    # Column headers
    header = f"{'#':>2} {'':2} {'Step Name':35s} | {'Actor':18s} | {'Avg':>6s} | {'ETA':>6s} | {'Progress':15s} | {'Activity 2h':21s} | Status"
...
def calculate_remaining_work(stats, total_postings):
    ...
    Excludes conditional steps (improve/regrade/ticket) since they only run for some postings.
    Handles fetch step specially - if any postings exist, fetch is complete.
    """
    remaining = 0
    for conv_id, name, actor, actor_type in WORKFLOW_STEPS:
        # Skip conditional steps - they don't run for all postings
        ...
        # Special case: Fetch step (conv 9144) - if postings exist, fetch is done
        if conv_id == 9144 and total_postings > 0:
            completed = total_postings
