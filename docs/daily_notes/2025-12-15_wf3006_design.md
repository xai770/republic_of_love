...
```
┌─────────────────────────────────────────────────────────────────┐
│                      WF3006: Entity Classifier                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐  │
│  │  FETCH   │───▶│ CLASSIFY │───▶│  DEBATE  │───▶│  APPLY   │  │
│  │ orphans  │    │  (LLM)   │    │ (opt.)   │    │ decision │  │
│  └──────────┘    └──────────┘    └──────────┘    └──────────┘  │
│       │                                               │         │
│       │              ┌──────────┐                     │         │
│       │              │  RESET   │◀────────────────────┘         │
...
│       └───────────────────┘                                     │
│                                                                 │
│  TRIGGER: Cron every 5 min OR on-demand                        │
│  BATCH SIZE: 25 skills                                          │
│  LOOP: Until no orphans remain                                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
2. If count > 0 → branch to FETCH (loop)
3. If count = 0 → end workflow
...
```
FETCH(25) → CLASSIFY(25) → [DEBATE(1) × N where needs_debate] → SAVE(25)
```
...
```
WF3006 Main Loop:
  FETCH → CLASSIFY → DEBATE → SAVE → APPLY → (loop to FETCH)

WF3006 Reset Entry (separate trigger):
  RESET → (skill returns to orphan pool) → Main Loop picks it up
```
...
| Aspect | Verdict |
|--------|---------|
| Data model | ✅ Option B is right |
| Self-continuation | ✅ Good design |
| Debate mechanism | ✅ Love it, keep it conditional |
| Reset flow | ⚠️ Separate from main loop |
| Quality metrics | ✅ Add the view now |
| Human-in-the-loop | ✅ Start with daily report, not UI |
...
## Update for Sandy: Prompt Testing Complete ✅
...
1. Created full workflow spec: [docs/workflows/3006_entity_classification.md](../workflows/3006_entity_classification.md)
   ...
   - Complete prompts for classify → challenge → defend → judge
...
|--------------|-------|--------|
| C3: Classify | gemma3:4b | ✅ Clean JSON, sensible classifications |
| C4: Challenge | qwen2.5:7b | ✅ Makes real challenges, not just "AGREE" |
| C6: Judge | gemma3:4b | ✅ Can side with challenger when warranted |

### The Big Win: Learning Works 🎯
...
- gemma3:4b (classifier): technology (confidence: 0.70)
  ...
  ⚠️ OVERTURNED: "SOX is a regulatory compliance framework, not a technology skill."
```
...
The model:
1. ✅ Correctly reclassified to `compliance_and_risk`
2. ✅ Explicitly referenced the mistake
3. ✅ Increased confidence from 0.70 → 0.95
4. ✅ Explained why the previous reasoning was wrong
...
### UPDATE 6:30 AM - Dynamic Domains Work! 🎯
...
- ✅ Full workflow design document (updated with dynamic domains)
- ✅ All prompts tested and working
- ✅ Data model defined (classification_reasoning table)
- ✅ Reset flow designed (C10)
- ✅ Domain creation flow designed (C3a)
- ✅ Domain split flow designed (C3b)
...
- Confidence jumps from 0.70 → 0.95
...
2. **Domain splits go through LLM review** - Same pattern as WF3001 job summaries: proposer → grader → second grader. If both graders agree the split makes sense, auto-approve. No human gate needed.

```python
# Domain split review flow (like WF3001 grading)
# C3b_propose: Model proposes split
# C3b_grade1: First grader reviews (qwen2.5:7b)
# C3b_grade2: Second grader reviews (gemma3:4b)
# 
# Auto-approve if: grade1.approve AND grade2.approve AND both confidence >= 0.8
# Otherwise: reject with reasoning, original domain stays intact
```
