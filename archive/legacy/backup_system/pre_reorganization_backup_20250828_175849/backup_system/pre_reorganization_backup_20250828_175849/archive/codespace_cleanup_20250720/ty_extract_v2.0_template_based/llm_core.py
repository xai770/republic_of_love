...
"""
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            # Parse template format instead of JSON
            result = {}
...
5. Return ONLY in this exact template format

ROLE_OVERVIEW: [Your 2-3 sentence summary here]
...
"""
        ...
        try:
            ...
            if response and len(response.strip()) > 20:
                # Parse template format: ROLE_OVERVIEW: [content]
                response = response.strip()
                
                if response.startswith("ROLE_OVERVIEW:"):
                    # Extract content after the template key
                    description = response[len("ROLE_OVERVIEW:"):].strip()
                else:
