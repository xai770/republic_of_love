-- Recipe 1123: Hybrid Job Skills Extraction
-- Extract skills from job postings with importance, proficiency, years, and reasoning

-- 1. Create Recipe
INSERT INTO recipes (recipe_id, recipe_name, recipe_description)
VALUES (1123, 'Hybrid Job Skills Extraction', 
        'Extract skills from job postings with importance (essential/critical/important/preferred), proficiency required (expert/advanced/intermediate/beginner), years required, and reasoning. Uses Qwen for intelligent skill analysis.')
ON CONFLICT (recipe_id) DO UPDATE
SET recipe_name = EXCLUDED.recipe_name,
    recipe_description = EXCLUDED.recipe_description;

-- 2. Create Session 1: Job Summary Extraction
INSERT INTO sessions (session_id, canonical_code, session_name, session_description, actor_id)
VALUES (3001, 'job_summary_extraction', 'Job Summary Extraction', 
        'Extract comprehensive job description summary', 'qwen2.5:7b')
ON CONFLICT (session_id) DO UPDATE
SET session_name = EXCLUDED.session_name,
    session_description = EXCLUDED.session_description;

-- 3. Create Session 2: Hybrid Skills Extraction
INSERT INTO sessions (session_id, canonical_code, session_name, session_description, actor_id)
VALUES (3002, 'job_hybrid_skills_extraction', 'Hybrid Skills Extraction', 
        'Extract skills with importance, proficiency, years, and reasoning', 'qwen2.5:7b')
ON CONFLICT (session_id) DO UPDATE
SET session_name = EXCLUDED.session_name,
    session_description = EXCLUDED.session_description;

-- 4. Link sessions to recipe
DELETE FROM recipe_sessions WHERE recipe_id = 1123;
INSERT INTO recipe_sessions (recipe_id, session_id, execution_order)
VALUES (1123, 3001, 1), (1123, 3002, 2);

-- 5. Create Instruction for Session 1 (Summary Extraction)
DELETE FROM instructions WHERE session_id = 3001;
INSERT INTO instructions (session_id, step_number, step_description, prompt_template, timeout_seconds)
VALUES (3001, 1, 'Extract job summary', $$You are analyzing a job posting to extract a comprehensive summary.
...
-- 6. Create Instruction for Session 2 (Hybrid Skills Extraction)
DELETE FROM instructions WHERE session_id = 3002;
INSERT INTO instructions (session_id, step_number, step_description, prompt_template, timeout_seconds)
VALUES (3002, 1, 'Extract skills with hybrid metadata', $$You are a skills extraction expert for a talent matching system. Extract skills from this job posting with precise metadata.
...
SELECT 'Recipe 1123 created successfully!' AS status;
