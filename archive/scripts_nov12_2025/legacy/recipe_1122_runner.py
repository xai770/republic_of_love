#!/usr/bin/env python3
"""
Recipe 1122 Runner: Profile Career Analysis
===========================================
...
USAGE:
    ...
    python3 recipe_1122_runner.py --profile-id 1
    
    # Analyze from file
    python3 recipe_1122_runner.py --career-file "docs/Gershon Pollatschek Projects.md"
    
    # Run with specific workflow_run_id for tracking
    python3 recipe_1122_runner.py --profile-id 1 --workflow-run-id 1234
...
class Recipe1122Runner:
    ...
    def get_profile_career_history(self, profile_id: int) -> tuple[str, dict]:
        ...
        career_doc_path = Path(f"docs/{profile['profile_name']} Projects.md")
    ...
    def create_workflow_run(self, profile_id: int) -> int:
        ...
        """, (self.recipe_id, 'RUNNING', f'Profile {profile_id} career analysis'))
    ...
    def log_llm_interaction(self, model: str, prompt: str, response: str, metadata: dict = None) -> int:
        ...
        """, (self.workflow_run_id, model, prompt, response, json.dumps(metadata) if metadata else None))
...
For each skill, explain WHY it is needed based on the specific responsibilities and organizational context described. Provide concrete examples from the career history."""
    ...
    def save_career_analysis(self, profile_id: int, deepseek_results: dict, 
        ...
        """, (
            ...
            json.dumps({'career_span': career_span, 'profile_name': profile_metadata['profile_name']}),
            'ensemble'
        ))
    ...
    def run(self, profile_id: int = None, career_file: str = None) -> bool:
        """
        Execute Recipe 1122: Profile Career Analysis
        ...
        """
        print("\n" + "="*70)
        print("🚀 Recipe 1122: Profile Career Analysis")
        print("="*70)
        ...
        try:
            ...
            if profile_id:
                ...
                with open(career_file, 'r') as f:
                    career_text = f.read()
                profile_metadata = {'profile_name': Path(career_file).stem}
                self.profile_id = None
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Recipe 1122: Profile Career Analysis')
