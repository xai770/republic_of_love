#!/usr/bin/env python3
"""
Create improved grader validation recipe with variable substitution and stricter prompt
"""
import sqlite3
import sys
...
# Create recipe for improved grader validation
canonical_code = "test_grader_validation_olmo2_v2"
review_notes = "Test olmo2:latest with improved prompt structure and variable substitution (7 test cases)"
...
# Create improved grading prompt template with variable substitution
...
# Create one session per test case with variable substitution
session_ids = []
for i, test_case in enumerate(TEST_CASES, 1):
    session_name = f"grade_v2_{test_case['name']}"
    session_desc = f"olmo2 v2 grades: {test_case['expected_grade']} - {test_case['reason']}"
...
# Create one instruction per session (same prompt template, different variations)
for session_id, test_case in session_ids:
    ...
    VALUES (?, 1, ?, ?, 1)
    """, (session_id, f"Grade v2 {test_case['name']}", improved_prompt))

print(f"\n✅ Created {len(session_ids)} grading instructions with variable substitution")

# Create one variation per test case (param_1 = job posting, param_2 = description to grade)
variation_ids = []
for i, test_case in enumerate(TEST_CASES, 1):
    cursor.execute("""
...
print(f"\n✅ Created {len(variation_ids)} variations with job posting + test descriptions")

# Create recipe_runs for each variation (one per test case)
recipe_run_ids = []
for i, (variation_id, test_case) in enumerate(zip(variation_ids, TEST_CASES), 1):
    ...
    VALUES (?, ?, ?, 'PENDING')
    """, (recipe_id, variation_id, i))  # Different batch_id for each
...
print("\n" + "="*70)
print("IMPROVED GRADER VALIDATION TEST READY")
print("="*70)
print(f"Recipe: {recipe_id} | Runs: {min(recipe_run_ids)}-{max(recipe_run_ids)}")
print(f"Sessions: {len(TEST_CASES)} (olmo2:latest with improved prompt)")
print(f"Variations: {len(variation_ids)} (job posting + test descriptions)")

print("\nImprovements:")
print("   ✅ Clear section headers with --- separators")
print("   ✅ Variable substitution: {variations_param_1} = job posting")
print("   ✅ Variable substitution: {variations_param_2} = test description")
print("   ✅ Stricter format: [PASS]/[FAIL] with [Explanation]")
print("   ✅ Concrete examples showing expected format")
print("   ✅ Multiple batch_ids to test different descriptions")

print(f"\nExpected results: 1 PASS, 6 FAIL")
print(f"Testing if improved prompt fixes olmo2's 86% false positive rate")
